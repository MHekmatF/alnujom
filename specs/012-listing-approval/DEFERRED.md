...
| **Q5=A Other-required UX gate** (T062 sub-step) | UI on Pixel | Reject dialog with "Other" preset selected and empty detail keeps Confirm DISABLED + flips field label to "Additional details (required)"; typing a non-empty detail enables Confirm; switching back to a non-Other preset flips the label back to "(optional)" while keeping Confirm enabled. FR-013(d) + SC-028 UX-side closed. |
