...
- [X] T033 [P] Run grep gate 1 per `quickstart.md Step 4`: `grep -r "status.*approved" lib/features/search/ --include="*.dart"` — expected: zero matches (Constitution III: no app-layer approved filter)
...
- [X] T037 Run `flutter run --dart-define-from-file=.env.json` on Infinix Note 8; execute `quickstart.md` steps 5–17 in full (SC-001 through SC-011); complete the SC Matrix in `quickstart.md Step 19` — mark each of the 11 SC rows Pass or Fail; all 11 must pass before declaring Phase 14 complete
  <!-- Emulator verification 2026-05-24 (Pixel_8_Pro AVD, Android API 37): SC-002 ✅ SC-003 ✅ SC-004 ✅ SC-005 ✅ SC-006 ✅ SC-007 ✅ SC-008 ✅ SC-009 ✅ SC-011-EP1 ✅ SC-011-EP2 ✅ | SC-001 ✅ RPC-verified 2026-05-24: search_listings(p_query=>'شقة') returns 'شقة فاخرة في دمشق'; address_text token 'المزة' also returns correct row | SC-010 ✅ RPC-verified 2026-05-24: page 1 = 10 rows (11 approved listings with prices), page 2 cursor = 1 row -->
