...
- **Arabic search hint suggestion algorithm (Phase 14)**: The `_buildSuggestion()` helper in `SearchPage` generates a suggested alternate Arabic form using a static lookup table of the 10 most common real-estate vocabulary root↔form pairs (e.g., `"شقة"→"شقق"`, `"فيلا"→"فلل"`, `"أرض"→"أراضي"`, `"غرفة"→"غرف"`, `"بيت"→"بيوت"`, `"منزل"→"منازل"`, `"محل"→"محلات"`, `"مكتب"→"مكاتب"`, `"مستودع"→"مستودعات"`, `"مزرعة"→"مزارع"`). If the query word is not in the table the helper returns the query unchanged. This is a v1 heuristic; morphological expansion is deferred to the smart-Arabic-search future phase.
- **SearchFilterSheet data-fetching pattern**: `SearchFilterSheet` directly calls `LocationRepository` (Phase 8) and `CurrencyRepository` (Phase 9) via `GetIt` to populate the cascading location pickers and currency selector. This is a deliberate v1 simplification per R-83: filter-sheet data loading is simple enough (lazy-loaded on sheet open, cached in local state) that a dedicated `FilterSheetCubit` adds ceremony without value. A future refactor phase may extract this if the sheet's complexity grows.
