...
- FC's registration seeding writes through **pre-existing** symbols — `ProfileRepository.updateLocale` (Phase 5) for the language and `CurrenciesRepository.writeUserDisplayCurrency` (Phase 9, `lib/features/currencies/domain/repositories/currencies_repository.dart`) for the currency; the new-listing visibility pre-selection consumes the pre-existing `ListingVisibility` entity (Phase 10). FD supplies only the *values* (via `LoadPublicSettings`).
