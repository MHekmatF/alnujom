...
- **Maintenance custom message language**: the maintenance screen's standard frame (title, retry, support label) is ARB-localized; an optional admin-set custom message MAY be provided per-locale and the active-locale variant is shown, falling back to the built-in localized message when no custom message is set.
...
- **FR-007**: The admin-set **default language** and **default currency** MUST seed a **new** user's preferences at account creation. They MUST NOT retroactively change any existing user's chosen `user_preferences.locale` / `display_currency` (the plan's "affects new users only" rule).
...
- **FR-011**: The maintenance screen MUST present a **localized message** (an admin-set custom message if provided, otherwise the built-in localized message), the configured **support contact**, and a **retry** affordance that re-checks the setting; turning maintenance OFF MUST restore normal access on the next check / retry.
...
- **FR-013**: The app MUST surface the **support contact** and the **terms / privacy links** from settings on the appropriate screen(s) (e.g., about / settings, and the maintenance screen), using the stored values; an unset link MUST degrade gracefully (hidden or a localized "unavailable" state), never a broken link.
...
- **Settings Catalog**: The closed v1 set of keys — default language, default currency, default publisher-name visibility, default exact-location visibility, maintenance mode (+ optional message), support contact, terms URL, privacy URL. Notably **excludes** "supported currencies" (Phase 9 `currencies.is_active` owns that).
...
- **Default Seeds**: The forward-only defaults — default language + default currency seed a **new** user's `user_preferences`; default publisher-name + exact-location visibility pre-select a **new** listing's visibility fields. Never retroactive.
...
- **Defaults are forward-only.** Default language/currency seed new users only; publisher-name/location-visibility defaults pre-select fields on new listings only. Existing users and listings are never retroactively rewritten (the plan's explicit rule).
...
- **The maintenance custom message is optional.** The maintenance screen's standard frame is ARB-localized; an optional admin-set custom message MAY be provided per-locale and shown for the active locale, falling back to the built-in localized message.
