...
- [ ] T027 [US3] Pre-select a **new** listing's `contact_name_visibility` / `location_visibility` from the `AppSettings` defaults in the new-listing initial state in `lib/features/listing_form/presentation/bloc/listing_form_bloc.dart` (consuming the `ListingVisibility` entity in `lib/features/listing_form/domain/entities/listing_visibility.dart`; existing listings untouched — FR-008)
