...
- **Two product decisions resolved** (see `## Clarifications`): (1) maintenance-mode bypass = `settings.manage` holders only (resolves the plan's §16 open question); (2) "supported currencies" is **not** a Phase 23 setting — Phase 9 `currencies.is_active` remains the source of truth; `app_settings` stores only the default currency.
- Ready for `/speckit-clarify` (optional — clarifications already resolved) or `/speckit-plan`.
