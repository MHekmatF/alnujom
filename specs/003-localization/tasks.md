...
- [ ] T003 [P] Expand `lib/l10n/app_en.arb` with the parallel English entries for every key landed in T002. The set of non-metadata keys in `app_en.arb` MUST be identical to `app_ar.arb` so the parity check (T013) will pass. English copy is professional and clear.
...
- [ ] T004 Run `flutter gen-l10n` from the repo root to regenerate `lib/l10n/app_localizations.dart`, `lib/l10n/app_localizations_ar.dart`, and `lib/l10n/app_localizations_en.dart` from the expanded ARB corpus. Commit the generated Dart files (the project uses `synthetic-package: false`, so generated files are checked in). Depends on T002, T003.
