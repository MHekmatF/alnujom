...
- [ ] T064 **⚠️ PARTIAL — backend applied; pending device walk.** Steps 1–2 DONE: all 8 migrations applied to Supabase (`apply_migration`, in order — `reports`+`moderation_actions` tables with RLS, the 2 SELECT policies, `v_reports` SECURITY INVOKER view, the resolution-audit trigger, and the `submit_report`/`resolve_report_internal`/`start_report_review` functions all verified present); `resolve_report` Edge Function deployed (ACTIVE v1, `verify_jwt` on — required adding the missing `supabase/functions/resolve_report/deno.json` import map); `get_advisors(security)` shows ZERO new findings — `v_reports` is correctly absent from the `security_definer_view` list, and `submit_report`/`start_report_review` carry only the same benign "exposed via API" advisory every prior-phase authenticated RPC (`add_favorite`, `submit_inquiry`, `record_lead_event`) carries (by-design; `resolve_report_internal` is service-role-only and carries none). **Remaining (step 3, needs a device):** execute the `quickstart.md` 13-step recipe on the Pixel 8 Pro AVD (412 dp) + Infinix Note 8 (480 dp) confirming SC-001..SC-015 (submit/anon-prompt, dedup, admin gating, 4 resolve actions + status map, sibling auto-resolve, dual-layer auth, read-matrix, reporter visibility, theme×locale). Tracked as the spec's QA handoff.
