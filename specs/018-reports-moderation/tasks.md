...
- [X] T064 **Device QA complete (Infinix Note 8, Android 10).** Backend applied (8 migrations + `resolve_report` Edge Function, `verify_jwt` on) and the full `quickstart.md` recipe walked on-device + verified at the wire level. **All 15 SCs PASS:** SC-001 submit (+ IP/UA metadata) · SC-002 dedup (+ re-report after resolution) · SC-003 admin gating + queue · SC-004 four actions → correct listing status + `moderation_actions` + `audit_logs` (7× `report.resolved` + listing transitions) · SC-005 actioned listings off the public view · SC-006 atomicity (failed call rolled back, zero partial state) · SC-007 self-scoped My Reports · SC-008 reporter-only banner · SC-009 wire-level cross-user isolation (regular user own-only, admin all, anon denied) · SC-010 dual-layer authZ (`resolve_report_internal` service-role-only; `submit_report`/`start_report_review` authenticated-only) · SC-011 anonymous prompt + no row · SC-012 theme×locale · SC-013 grep gates (T065) · SC-014 paginated queries · SC-015 double-resolve guard (`already_resolved`). FR-016 sibling auto-resolve and FR-036 soft-claim also confirmed. **6 bugs found & fixed during QA** (commits on branch): resolve-button mislabel; `resolve_report_internal` `listing_id` ambiguity; `v_reports` invoker→definer scoping (reporters lost reports on non-approved listings); My-Reports/banner self-scope; `anon` EXECUTE revoke; missing Edge-Function `deno.json`. **Open cosmetic nit (backlog, non-blocking):** resolved queue/detail cards still show "Being reviewed by …" (present tense) because `reviewing_by` isn't cleared on resolve.
