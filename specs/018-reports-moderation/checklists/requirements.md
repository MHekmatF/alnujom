...
- All three product-shaping ambiguities (moderation-action/status mapping, reporter authentication, reporter-visibility surface) were resolved with the user during `/speckit-specify` and folded into the spec; see the Clarifications section. No open clarification markers remain.
- Two narrowly-scoped, non-blocking implementation choices are explicitly deferred to `/speckit-plan` and flagged in Assumptions (the `reporter_user_id` ON DELETE behavior; whether `submit_report` requires `approved`-only vs. existence-only). These do not block planning.
