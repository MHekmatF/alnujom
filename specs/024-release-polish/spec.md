...
**Independent Test**: In a profile/release build with the DSN set (crash reporting is release/profile-only — a pure debug build sends nothing), deliberately trigger an uncaught exception and confirm it appears on the crash dashboard within ~5 minutes with a usable stack trace. Inspect the report payload and confirm it contains **no** secrets (no Vault material), **no** synthetic-email→phone mapping, and **no** decrypted PII. Take the dashboard offline / unreachable and confirm the app still starts and runs normally.

**Acceptance Scenarios**:

1. **Given** a profile/release build (DSN set) wired to the crash dashboard, **When** an uncaught exception is forced, **Then** the error appears on the dashboard with a usable stack trace within ~5 minutes.
...
- **FR-005**: A **deliberately-forced exception in a profile/release build (with the DSN set)** MUST appear on the crash dashboard with a usable stack trace (the plan's "captures a forced exception in dev build" acceptance criterion — note crash reporting is **release/profile-only** per R-217, so a pure debug build sends nothing; verification uses a profile build with the DSN).
...
- **SC-002**: A deliberately-triggered fatal error in a **profile/release build (DSN set)** appears on the crash dashboard within ~5 minutes with a usable stack trace, and the report payload contains **no** secrets, **no** synthetic-email→phone mapping, and **no** decrypted PII — verified by inspecting the dashboard entry.
