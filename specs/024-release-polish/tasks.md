...
- [ ] **⚠️ PARTIAL —** T019 [US4] Run `build_runner` + `flutter analyze`; grep-confirm no `supabase_flutter`/`package_info_plus` import under `lib/features/app_update/domain/` — **acceptance (record outcome)**: higher manifest ⇒ prompt + download; equal/older/unreachable ⇒ silent no-op; four-combination render (SC-003/SC-007). **Code-level checks PASS** (`flutter analyze` clean; build_runner clean; grep clean; 225 existing tests pass). On-device walk deferred to QV.
