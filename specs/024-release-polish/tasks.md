...
- [X] T001 [P] [US2] Add `sentry_flutter` to `dependencies` in `pubspec.yaml` (Android-supported; R-206)
- [X] T002 [P] [US2] Create the abstract `CrashReporter` seam in `lib/core/logging/crash_reporter.dart` (`init({dsn, environment})`, `recordError(error, stack, {context})`, `addBreadcrumb`, `close`) per `contracts/phase24-crash-reporting-and-logger-seam.md`
- [X] T003 [US2] Create `SentryCrashReporter` (`@LazySingleton(as: CrashReporter)`) in `lib/core/logging/sentry_crash_reporter.dart` — wraps `sentry_flutter`; installs the **`beforeSend` scrub** stripping synthetic-email/phone, Vault material, decrypted PII, and tokens (data-model §2) (depends on T001, T002)
- [X] T004 [P] [US2] Create `NoopCrashReporter` (`implements CrashReporter`, all no-ops) in `lib/core/logging/noop_crash_reporter.dart` — bound when the DSN is empty (mirrors `NoopPushMessagingService`) (depends on T002)
- [X] T005 [US2] Amend `lib/main.dart` — read `SENTRY_DSN` (dart-define); **guard-init** Sentry in `try/catch` (Phase 22 Firebase-guard pattern), wrap the bootstrap in `runZonedGuarded`, route `FlutterError.onError` + `PlatformDispatcher.onError` → `CrashReporter.recordError`; bind `NoopCrashReporter` when the DSN is empty; init MUST NOT block `runApp` (FR-007); release/profile only (depends on T003, T004)
- [X] T006 [P] [US2] Add a `SENTRY_DSN` key to `.env.example.json` (committed) + a one-line doc note; do **NOT** commit a real DSN (R-217, ADR-0001)
- [X] T007 [US2] Run `dart run build_runner build --delete-conflicting-outputs` + `flutter analyze`; grep-confirm **no** `package:sentry_flutter` import under any `domain/` — **acceptance (record outcome)**: in a **profile build with the DSN set**, force an uncaught exception via a **`--dart-define`-gated debug-only "throw test error" affordance** (inert/removed in the shipped release) → confirm it lands on the dashboard; payload inspected (no secrets/PII); unreachable dashboard → app runs (SC-002). **VERIFIED 2026-06-03 (signed release build on Pixel 8 Pro AVD → GlitchTip, a Sentry-compatible endpoint):** the `SENTRY_TEST_CRASH`-gated affordance fired an uncaught `StateError` → landed on the dashboard as issue `ALNUJOM-APP-1` with full stack trace + device + release `com.alnujom.app@1.0.0+1` + breadcrumbs; **payload scrubbed — no phone/email/token/Vault** (only a random per-install UUID, not PII); the app **survived** the crash (non-blocking, FR-007); empty-DSN ⇒ Noop ⇒ app fine (all prior builds). Code checks (build_runner/analyze/grep — no `sentry_flutter` under `domain/`) PASS. Secret hygiene: `SUPABASE_SERVICE_ROLE_KEY` moved out of `.env.json` → gitignored `.env.admin.json` (APK scans confirm only the anon key ships).
...
- [X] T008 [P] [US4] Add `package_info_plus` to `dependencies` in `pubspec.yaml` (Android-supported; R-210)
- [X] T009 [P] [US4] Create domain entities in `lib/features/app_update/domain/entities/`: `app_version.dart` (`AppVersion` {major,minor,patch,build} + `compareTo` semver-first/build-tiebreaker + `parse`), `version_manifest.dart` (`VersionManifest`), `update_availability.dart` (sealed `UpdateAvailability`: `UpdateAvailable`/`UpToDate`/`CheckFailed`) per data-model §3
- [X] T010 [P] [US4] Create abstract `AppUpdateRepository` in `lib/features/app_update/domain/repositories/app_update_repository.dart` (`Future<Result<UpdateAvailability>> checkForUpdate()` — never throws)
- [X] T011 [US4] Create the `CheckForUpdate` use case in `lib/features/app_update/domain/usecases/check_for_update.dart` (depends on T010)
- [X] T012 [P] [US4] Create `VersionManifestDto` (+ tolerant JSON decode) in `lib/features/app_update/data/dtos/version_manifest_dto.dart` (data-model §1); **also commit the checked-in canonical schema template** `docs/release/version-manifest.example.json` (Principle II/XII — the operator uploads the live copy to Supabase Storage; the DTO MUST decode this example) — satisfies the plan's "manifest content checked-in" claim
- [X] T013 [US4] Create `SupabaseManifestDatasource` (Storage GET of the public manifest via the existing `SupabaseClientWrapper`) + `PackageInfoVersionSource` (installed version via `package_info_plus`) in `lib/features/app_update/data/datasources/` (depends on T008, T012)
- [X] T014 [US4] Create `AppUpdateRepositoryImpl` (`@LazySingleton(as: AppUpdateRepository)`) in `lib/features/app_update/data/repositories/app_update_repository_impl.dart` — **fail-silent** (`CheckFailed`) on unreachable/malformed (FR-010) (depends on T010, T013)
- [X] T015 [US4] Create `AppUpdateCubit` in `lib/features/app_update/presentation/bloc/app_update_cubit.dart` — `check()` → emits `UpdateAvailable`/`UpToDate`/`CheckFailed` (depends on T011)
- [X] T016 [P] [US6] Add the update-prompt l10n keys (title, body, Update, Later, optional release-note label) to `lib/l10n/app_ar.arb` + `lib/l10n/app_en.arb` + matching `_DebugAppLocalizations` overrides in `lib/core/localization/app_strings.dart`; run `flutter gen-l10n` (memory `project_l10n_debug_localizations_override`)
- [X] T017 [US4] Create `update_prompt_dialog.dart` in `lib/features/app_update/presentation/widgets/` — localized; **Update** (`url_launcher` → `telegram_url` ?? `website_url`) + **Later** (dismiss for session); Phase 2 tokens (depends on T015, T016)
- [X] T018 [US4] Amend `lib/app.dart` — provide `AppUpdateCubit`, call `check()` **once on cold start** (not on every resume), and show `update_prompt_dialog` on `UpdateAvailable` (session-once; R-211) (depends on T015, T017)
- [ ] **⚠️ PARTIAL —** T019 [US4] Run `build_runner` + `flutter analyze`; grep-confirm no `supabase_flutter`/`package_info_plus` import under `lib/features/app_update/domain/` — **acceptance (record outcome)**: higher manifest ⇒ prompt + download; equal/older/unreachable ⇒ silent no-op; four-combination render (SC-003/SC-007). **Code-level checks PASS**. **Higher-manifest ⇒ prompt VERIFIED 2026-06-03 on the signed release build + Pixel 8 Pro AVD** (real manifest `1.1.0` in Supabase Storage `app-release/android/latest.json` → localized "Update Available" dialog with release notes + Update/Later, over onboarding). **This surfaced + fixed a real bug** (commit `a98c1be`): the prompt used the MaterialApp.router builder context (no Navigator) so `showDialog` silently failed; fixed via `rootNavigatorKey`. **Equal (1.0.0) ⇒ no prompt** and **unreachable (HTTP 400/not_found) ⇒ no prompt + no crash** also VERIFIED 2026-06-03 (signed release on AVD) — full higher/equal/unreachable matrix passes (SC-003 / FR-010 fail-silent). **Four (light/dark)×(ar/en) renders VERIFIED 2026-06-03** (app home captured in all four combos on the signed release — RTL fully flips incl. the + FAB + category order, full Arabic l10n with Arabic-Indic numerals + relative time, clean dark theme; the prompt inherits the same `MaterialApp` theme + `AppStrings` — SC-007). **Remaining**: Update→Telegram launch (minor).
...
- [ ] T024 [US1] **⚠️ PARTIAL —** `pubspec.yaml` version `1.0.0+1`; `debugShowCheckedModeBanner: false`; `kDesignToolsEnabled` off in release. **VERIFIED 2026-06-03 on a fresh Pixel 8 Pro AVD**: signed release APK builds (70.8 MB), signed with the **release key** (apksigner `CN=Hekmat Fanari, OU=AlNujom` — not debug), **fail-closed** confirmed (no `key.properties` → Gradle config error, no debug fallback), **boots clean** (login + navigation work, no debug banner). **Remaining**: real branding art (icon/splash still placeholder blue star) + optional Infinix Note 8 boot. (depends on T021, T022, T023)
...
- [ ] T030 [US6] Run `flutter analyze` — **acceptance (record outcome)**: exercise the AdSlot / AgencyVerification / Profile navigations with fast back-nav → **no** "emit after close"; open an approved agency → logo renders (SC-009). **⚠️ PARTIAL —** **agency logo renders VERIFIED 2026-06-03** (the مكتب النجوم agency badge + logo render in the live home feed on the signed build — the double-prefix fix is confirmed in production data); the fast-back-nav "emit after close" exercise is still pending on-device. (depends on T025–T029)
...
- [X] T031 [US3] Write `integration_test/primary_publish_path_test.dart` — the one automated test: register → admin-approve → publish → admin-approve → public view → inquiry (drives pre-existing Phase 5/10/12/13/16 surfaces). **Include a SETUP/teardown that establishes the test fixture**: a fresh registerable phone (the synthetic-email pattern), a **pre-seeded admin** holding `users.approve` + `listings.approve` (the admin-side approvals driven via service-role/SQL between UI steps), deterministic listing data, and a reset so the test is **re-runnable** (idempotent against the live/staging backend). **Written and `flutter analyze` clean.** Credentials supplied via dart-defines (TEST_ADMIN_PHONE / TEST_NEW_USER_PHONE etc.). On-device green run = T032 (PARTIAL below).
- [ ] **⚠️ PARTIAL —** T032 [US3] Run `flutter test integration_test/primary_publish_path_test.dart --dart-define-from-file=.env.json` on the Pixel 8 Pro AVD — **acceptance (record outcome)**: green (depends on T031, Wave 1 merged). Needs the merged build + a seeded backend. T031 (test file) is DONE; this task tracks the actual execution.
- [ ] **⚠️ PARTIAL —** T033 [US3] **six manual golden-path walks** — walked on the Pixel 8 Pro AVD 2026-06-03: **(1) primary publish — PASS** (agent-observed; register+publish+approve+view+inquiry via UI; first account/publisher approval DB-bootstrapped since no admin existed — re-verify that sub-step via UI later); **(2) browse+filter+map — PASS** (operator); **(3) admin reports-queue resolution — PASS** (operator; report filed + resolved); **(4) super-admin role create+assign+revoke — PASS** (operator); **(5) currency switch + exchange-rate update — PASS** (operator). **Remaining/caveated**: **(6) maintenance + recovery — PASS on AVD (operator)** but the **two-device** "second device recovers" half still needs a real 2nd device; paths 2–5/6 are operator-attested (agent observed only #1). — **acceptance (SC-004)**.
- [X] T034 [US5] Build the signed release APK and run a **binary secret-scan** of it (Phase 22 T046) — **DONE 2026-06-03 (agent-performed on the 70.8 MB signed app-release.apk): CLEAN.** Unzipped + grepped contents (incl. libapp.so) + decoded every embedded JWT: exactly one JWT, role=`anon` (no `service_role`/`sb_secret_`); no Sentry DSN, no keystore material (`*.jks`/passwords), no FCM service-account/private keys, no Vault material. Only the Supabase URL + public anon key ship (by design). — **acceptance: PASS (SC-006)**.
- [ ] **⚠️ PARTIAL —** T035 [US3] Measure + record the **cold-start baseline** on the Infinix Note 8 vs the §15 < 3 s budget — **advisory, not a gate** (clarify Q2) — **acceptance (record value)**. On-device.
- [X] T036 [US5] Author `docs/release/v1.0.0.md` — the six golden-path results, install/update instructions, the **no-email account-recovery support flow** (admin issues a temp password via the super-admin UI, §15), the cold-start baseline, the APK secret-scan result, the distribution checklist, **and the operator prerequisites that gate the release** (the Sentry self/EU instance + DSN, the release keystore, the Telegram channel) (FR-012). **Draft authored at `docs/release/v1.0.0.md`; all device/operator-dependent values clearly marked PENDING.**
- [ ] **⚠️ PARTIAL —** T037 [US5] **Distribution ops** (R-216): upload the signed APK + the version manifest JSON to Supabase Storage; post the APK to the Telegram channel; configure the Play Store internal testing track (QA-only) — **acceptance (SC-005)**. Operator step.
- [X] T038 [US3] Run the **full verify suite** (`flutter analyze --fatal-infos` + design-tokens + l10n-parity + l10n-literals — memory `project_wave_run_full_verify_suite`) + the **structural gate** (quickstart §G: version `1.0.0`; **no** new table/RPC/permission/migration; **no** `supabase_flutter`/`sentry_flutter`/`package_info_plus` import under any `domain/`; **no** iOS/Web; carried-over buckets closed, FE-1 left future); out-of-scope findings recorded in `specs/024-release-polish/DEFERRED.md` — **acceptance (SC-008)**. **ALL PASS**: analyze clean (No issues found); 225 tests pass; design-token lint 3 (baseline, pre-existing); l10n-parity PASS (875 keys); l10n-literals 9 (baseline, pre-existing); no new Phase-24 migration; no domain/ platform imports; no iOS/Web code; isClosed guards present; logo fix merged; FE-1 left future.
- [ ] **⚠️ PARTIAL —** T039 [US3] Verify the **Phase 22 carried-over QA residual** (FR-013 / R-215 — beyond the T034 APK secret-scan): **(a) T044** — admin counters move ≤5 s + reconcile after a forced network drop on a 2nd device (the live role grant/revoke half is already VERIFIED PASS — re-confirm on the release build); **(b) T045** — all six in-app notification events deliver (center + badge) with push **blocked**, no crash; **(c) T046** — cross-user RLS-denial from a **real user-X JWT session** + non-admin Realtime delivers **no** admin rows — record each — **acceptance (SC-004; closes the FR-013 "fix all" mandate)**. Two-device / on-device + a real user-X session.
