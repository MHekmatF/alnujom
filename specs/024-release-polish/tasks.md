...
- [ ] **⚠️ PARTIAL —** T033 [US3] Run the **six manual golden-path walks** (Infinix Note 8 + Pixel 8 Pro AVD): **(1) primary publish — PASS 2026-06-03 on Pixel 8 Pro AVD** (register + publish + listing-approve + public view + inquiry all via UI; caveat: first account+publisher approval bootstrapped in Supabase since no admin existed — re-verify that sub-step via UI with a future test account); (2) anonymous browse+filter+map — pending; (3) admin reports-queue resolution — pending; (4) super-admin role create+assign+revoke — pending; (5) currency switch + exchange-rate update — pending; (6) **maintenance mode + recovery (two-device)** — pending; record each — **acceptance (SC-004)**. On-device.
