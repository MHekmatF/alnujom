...
- [ ] T007 [US2] Run `dart run build_runner build --delete-conflicting-outputs` + `flutter analyze`; grep-confirm **no** `package:sentry_flutter` import under any `domain/` — **acceptance (record outcome)**: in a **profile build with the DSN set**, force an uncaught exception via a **`--dart-define`-gated debug-only "throw test error" affordance** (inert/removed in the shipped release) → confirm it lands on the dashboard; payload inspected (no secrets/PII); unreachable dashboard → app runs (SC-002). **⚠️ PARTIAL —** the **`--dart-define`-gated forced-crash affordance is now BUILT** (`SENTRY_TEST_CRASH` flag in `app.dart` → uncaught async throw 3 s after start, routed through `runZonedGuarded`/`PlatformDispatcher.onError` → `recordError`; inert in the shipped release). Empty-DSN ⇒ Noop ⇒ app runs is already true (current builds). **Remaining (needs a real DSN — operator must provision a Sentry-compatible endpoint):** profile build with the DSN + `SENTRY_TEST_CRASH=true` → confirm the event lands on the dashboard + payload scrubbed (no PII).
