...
- **Suggested merge order: CR → UP → RB → CF, then QV last.** (CR/UP/RB/CF order among themselves is free — only `pubspec.yaml` + `injection.config.dart` need union/regfen; pick the order that minimizes pubspec re-merges.) **QV merges last** and runs the full verification on the integrated, signed build.
