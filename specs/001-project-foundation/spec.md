...
- **FR-015**: The repository MUST include a hosted CI pipeline that, on every push to a feature branch and on every pull request targeting `main`, runs (a) static analysis, (b) the unit/widget test suite (including the smoke test from FR-012), and (c) a debug Android APK build. Pipeline failure MUST block merge; pipeline success MUST be visible on the PR status surface.
