...
- [X] T056 [P] Run the WCAG 2.1 AA spot-check from quickstart.md §9 manually on the Infinix Note 8: enable TalkBack, navigate the shell, verify both toggles announce purpose + current state; bump system font size to 130%, verify no truncation; sample contrast on toggle text, brand, and icons against their backgrounds with a contrast meter or `Color.computeLuminance()` check (FR-017)
  ...
  - **Phase 6 check 2026-04-30 (PASS)**: Verified on Infinix X692. TalkBack service enabled via Android accessibility settings for the check; UI hierarchy exposed focusable semantics `Current theme: dark, Toggle theme` and `Current language: en, Toggle language`, so both controls announce purpose + current state. At 130% font scale, brand bounds `[274,653][446,747]`, theme toggle `[48,795][389,891]`, locale toggle `[413,795][720,891]`; no overflow or missing control. Contrast ratios computed from token colors: light brand/onSurface vs surface 15.63:1, light toggle primary vs surface 6.73:1, dark brand/onSurface vs surface 15.44:1, dark toggle primary vs surface 10.20:1. Restored font scale and TalkBack settings after verification.

- [X] T057 [P] Verify Constitution Principle XII grep-style guards locally and in CI: `grep -rE "package:supabase_flutter" lib --include='*.dart' --exclude='supabase_client_wrapper_impl.dart'` returns 0; `grep -rE "Color\\(0x" lib --exclude-dir=l10n` returns matches only inside `lib/core/theme/`
  - **Verify**: both checks pass locally; CI step from T010 reproduces them; if any new violation appears, fix before merging
  - **Phase 6 check 2026-04-30 (PASS)**: Local search found `package:supabase_flutter` only in `lib/core/network/supabase_client_wrapper_impl.dart`; local color-literal search found matches only in `lib/core/theme/tokens_stub.dart`. Updated `.github/workflows/ci.yml` to also run the color-token boundary guard and to trigger on `001-*` feature-branch pushes.

- [X] T058 Update root `README.md` (create if absent) with a one-paragraph project description, a link to `docs/IMPLEMENTATION_PLAN.md`, and a one-line pointer to `specs/001-project-foundation/quickstart.md` for the Phase 1 verification recipe
  - **Verify**: README renders in GitHub's preview without broken links
  - **Phase 6 check 2026-04-30 (PASS)**: Replaced the generated Flutter README with a concise AlNujom project description and verified both repository-relative links point to existing files.

- [X] T059 Run the full `specs/001-project-foundation/quickstart.md` validation start-to-finish on a fresh clone to confirm SC-001 (≤30 minutes from clone to running emulator with no undocumented manual steps); record actual time in the PR description
  - **Verify**: SC-001 met; if any documented step is wrong or missing, fix the quickstart before declaring done
  - **Phase 6 check 2026-04-30 (PASS)**: Normal file clone to `C:\Users\Hekmat\AppData\Local\Temp\alnujom-phase6-20260430042551` completed; `flutter pub get`, `dart run build_runner build --delete-conflicting-outputs`, `flutter test`, `flutter build apk --debug --dart-define=SUPABASE_URL='' --dart-define=SUPABASE_ANON_KEY=''`, `adb install -r`, and `adb shell am start` all completed. UI hierarchy showed the shell brand and both toggle semantics. Actual elapsed time was approximately 3.3 minutes from clone start to running shell, under the 30-minute SC-001 budget.

- [X] T060 Run `/speckit-analyze` on the feature folder to confirm cross-artifact consistency (spec.md ↔ plan.md ↔ data-model.md ↔ contracts/ ↔ tasks.md); resolve any finding before merge
  - **Verify**: `/speckit-analyze` reports no inconsistencies; any finding either fixed or recorded as a deferred follow-up issue
  - **Phase 6 check 2026-04-30 (PASS)**: Cross-artifact analysis found no critical/high inconsistencies after the CI guard update. Requirements FR-001 through FR-017 and SC-001 through SC-006 are covered by tasks T001 through T061; Constitution checks remain aligned with the plan and contracts.

- [X] T061 Configure GitHub branch protection on `main`: require the `verify` workflow status check to pass before any PR can merge; require linear history; disallow direct pushes (FR-015 "Pipeline failure MUST block merge"). This is a one-time admin action via the GitHub repo Settings → Branches UI (or `gh api -X PUT repos/:owner/:repo/branches/main/protection ...` for an automatable equivalent)
  ...
  - **Phase 6 check 2026-04-30 (PASS)**: `gh api repos/MHekmatF/alnujom/branches/main/protection` reports required status checks with context `verify`, `strict: true`, `required_linear_history.enabled: true`, `allow_force_pushes.enabled: false`, and pull-request review protection present. This satisfies FR-015 merge-blocking enforcement for `main`.
