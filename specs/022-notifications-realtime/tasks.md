...
- [ ] T044 [US3] [US4] **⚠️ PARTIAL — US4 permission refresh VERIFIED live; US3 admin counters deferred.** SC-005 (US4) PASSED on-device 2026-06-02 (Pixel 8 Pro AVD): granting a role surfaced the gated Home admin entry live and revoking removed it live, no re-login — after fixing two live-only bugs (`REPLICA IDENTITY FULL` on `user_roles` + `AuthBloc` re-emit; see DEFERRED.md §4). The existing 3 PermissionChecker points + the resume path also confirmed. REMAINING: SC-004 admin counters (needs an admin login + a 2nd session). Two-device walk: admin counters (pending listings, reports) move ≤5 s with no refresh + reconcile after a forced network drop (SC-004); granting/revoking a role on device B updates device A's gated UI without re-login (SC-005); confirm the existing 3 observation points still work.
