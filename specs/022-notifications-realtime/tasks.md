...
- [X] T028 [P] [US1] `lib/features/notifications/presentation/bloc/notifications_cubit.dart` — states (loading/list/paginating/error); drives `LoadNotifications`/`MarkNotificationRead`/`MarkAllNotificationsRead`; bounded pagination.
- [X] T029 [P] [US1] `lib/features/notifications/presentation/bloc/notification_badge_cubit.dart` — unread count via `LoadUnreadCount`; refresh on mount + foreground-resume + when the push listener signals a foreground message (NOT a Realtime subscription — R-193).
- [X] T030 [P] [US1] [US6] `lib/features/notifications/presentation/widgets/notification_tile.dart` — read/unread styling, relative time, type icon; Phase 2 tokens + direction-aware (no inline hex/font/padding).
- [X] T031 [US1] `lib/features/notifications/presentation/widgets/notification_deep_link_resolver.dart` — maps `NotificationType`+`params` → pre-existing routes (account_approved→`AppRoutes.home`; account_rejected→Phase 5 rejected route; listing_approved→listing-details; listing_rejected→publisher My-Listings; inquiry_received→Phase 16 inquiries; agency_invitation→`/agency`); marks the notification read on navigation; graceful "content unavailable" fallback on unresolved target (FR-007/FR-018). (Imports PD `NotificationType`.)
- [X] T032 [US1] `lib/features/notifications/presentation/pages/notification_center_page.dart` — newest-first list, unread styling, mark-all-read action, tap → resolver + `MarkNotificationRead`, empty/loading/error states. (Depends on T028, T030, T031.)
- [X] T033 [US1] `lib/features/notifications/presentation/widgets/notification_bell_action.dart` — `IconButton` + unread-count badge from `NotificationBadgeCubit`, routes to `/notifications`. (Depends on T029.)
- [X] T034 [US1] Route: `lib/core/routing/app_router.dart` — `AppRoutes.notifications='/notifications'` + `AppRouteNames.notifications` + a `GoRoute` (name+builder→`NotificationCenterPage`) in the authenticated branch of `buildAppRouter()`.
- [X] T035 [US1] Insert `NotificationBellAction()` into `lib/features/home/presentation/pages/home_page.dart` AppBar `actions` — between `InquiriesAppBarAction()` and the admin-panel button (R-192). (Depends on T033, T034.)
- [X] T036 [US1] [US2] App-level push listener `lib/features/notifications/presentation/widgets/notification_push_listener.dart` — subscribes to the PD `PushMessagingService` streams: `onMessageOpenedApp` + `initialMessage()` (cold-start) → route via `NotificationDeepLinkResolver` (marks read on nav — FR-012/SC-002); `onMessage` (foreground) → refresh `NotificationBadgeCubit`/center, OPTIONAL snackbar, NO duplicate system banner (FR-014). Registered at the app shell/router so it is live app-wide; inert when the no-op push adapter is bound (push unconfigured — FR-013). (Depends on PD T024 + T031, T029.)
- [X] T037 [P] [US1] [US6] Add ~40 notification l10n keys (center labels, empty/loading/error, mark-all-read, relative-time, bell tooltip, the foreground/unavailable affordance strings) + the 6 GENERIC bilingual in-app copy templates (one per `type`) to BOTH `lib/l10n/app_ar.arb` + `lib/l10n/app_en.arb`; run gen-l10n (regenerates `app_localizations*.dart`).
- [X] T038 [US1] DI regen (`dart run build_runner build --delete-conflicting-outputs`) for `NotificationsCubit` + `NotificationBadgeCubit` (+ the listener if DI-registered); wire the listener into the app shell/router; `flutter analyze` + l10n-parity clean. (Depends on T028–T037.)
...
- [ ] T043 [US1] [US2] **⚠️ DEFERRED — human two-device QA (see DEFERRED.md).** Requires two physical devices + a real Firebase/FCM config + registered Vault secrets — not performable by the orchestrator. Two-device walk: trigger all six events → each lands once in the recipient's center ≤5 s with a working deep link (SC-001); with push configured, a system push arrives on device A ≤10 s incl. a cold-start tap (SC-002); verify exactly-once on retry (SC-009); the unread **badge count matches** unread rows and mark-read (single + all) updates it + history persists across logout/login and a fresh reinstall on a second device (SC-008); a token is **registered on login / removed on logout** and with two devices both receive push while logging out one leaves the other (SC-011).
- [ ] T044 [US3] [US4] **⚠️ PARTIAL — US3 + US4 core PASSED live; only network-drop reconcile remains.** Both verified on-device 2026-06-02 (Pixel 8 Pro AVD): **SC-005 (US4)** — granting a role surfaced the gated Home admin entry live and revoking removed it live, no re-login (after the two live-only fixes: `REPLICA IDENTITY FULL` on `user_roles` + `AuthBloc` re-emit — DEFERRED.md §4); resume-path cache reload also confirmed. **SC-004 (US3)** — the admin dashboard "Reports" counter ticked 0→1→0 live via a backend report insert/delete with no refresh (debounced `DashboardCubit` refetch; confirms `listings`/`reports` unfiltered subscriptions deliver on default replica identity). REMAINING: the forced-network-drop reconcile-on-resubscribe path (hard to exercise on the emulator). Two-device walk: admin counters (pending listings, reports) move ≤5 s with no refresh + reconcile after a forced network drop (SC-004); granting/revoking a role on device B updates device A's gated UI without re-login (SC-005); confirm the existing 3 observation points still work.
- [ ] T045 [US2] **⚠️ PARTIAL — build + skip-logic verified; on-device in-app delivery deferred.** VERIFIED: PD's `flutter build apk --debug` succeeded with NO `google-services.json`; `notify_push_dispatch` returns without posting when either dispatch secret is NULL (degraded — confirmed in the applied SQL, no Vault secrets registered); `NoopPushMessagingService` binding path reviewed. REMAINING (needs a device): all six events still deliver in-app (center + badge) with no error/crash. Degraded mode: with NO Vault secrets registered (Firebase "blocked"), `flutter build apk --debug` succeeds and all six events still deliver in-app (center + badge) with no error/crash; `notify_push_dispatch` skips silently and the no-op push adapter binds (SC-003).
- [ ] T046 [US5] **⚠️ PARTIAL — static/posture security VERIFIED; live session + APK-artifact scan deferred.** VERIFIED (live + static): both tables RLS-on + ALL client writes REVOKEd; self-only SELECT policies; `enqueue_notification`/`notify_push_dispatch` NOT client-EXECUTE-able (PUBLIC grant revoked in `…120012` after live `has_function_privilege` caught the hole) so a client CANNOT forge a notification for user Y; `grep` of `lib/ android(src)/ supabase/functions/dispatch_push` shows only Vault-name reads (no key material; client has ZERO secret references); no service-account JSON committed; `notifications_enabled=false` → `dispatch_push` returns `{skipped:'muted'}` while `enqueue_notification` still writes history (FR-021). REMAINING (needs runtime): cross-user RLS-denial from a real user-X JWT session; built-APK binary scan for secrets; non-admin Realtime delivers no admin rows. Wire-level security: from user X's session, reading/writing user Y's `notifications`/`notification_tokens` and inserting a notification for Y are all DENIED; `grep -ri "fcm_service_account\|push_dispatch_token" lib/ android/ supabase/functions/dispatch_push` shows only Vault *reads* (no key material) and the built APK contains no service-account JSON or dispatch token; non-admin Realtime delivers no admin rows (SC-007). Confirm `notifications_enabled=false` mutes push but still writes history (FR-021).
- [ ] T047 [US6] **⚠️ DEFERRED — human on-device render QA (see DEFERRED.md).** Static l10n-parity (825 keys) + 0 new design-token/literal violations VERIFIED; visual four-combination render + push-tray locale need a device. Four-combination render (light/dark × ar-RTL/en-LTR) of the center (list, tile, unread badge, empty/loading/error) + the bell on the Infinix Note 8 + a 412 dp Pixel 8 Pro AVD; push-tray text appears in the recipient's preferred language (default Arabic), and the OS-tray body is GENERIC (no rejection reason — FR-004) while the full reason shows in-app (SC-006).
- [ ] T048 [US5] **⚠️ PARTIAL — all static gates + full lint suite VERIFIED; live quickstart deferred.** VERIFIED: `package:firebase` only in `notifications/data` (`fcm_push_messaging_service.dart`) + `main.dart` composition root, NONE under any `domain/` and the `core/messaging` interface is SDK-free (Principle IX); no `supabase_flutter` in any `domain/`; no iOS/Web files; Android-only config; no new permission key (no permission-catalog file touched — FR-025); backend surface = the enumerated set **+ one corrective migration `…120012`** (PUBLIC-grant revoke — see DEFERRED.md); `dart run build_runner build` clean; full CI suite — `flutter analyze --fatal-infos` clean, `dart format` (Phase 22 files) clean, design-tokens + l10n-literals (0 NEW violations; 12 pre-existing in agency/reports baseline), l10n-parity (825 keys), SDK-boundary all pass (SC-010). REMAINING: execute `quickstart.md` end-to-end on devices (needs Firebase + two devices). Reconcile spec/plan/data-model/contracts if any real behavior diverged (Principle X) — see DEFERRED.md for the 2 divergences (`…120012` migration; `RealtimeSignals` seam). Structural/dependency gates + full verify suite; execute `quickstart.md` end-to-end; reconcile spec/plan/data-model/contracts if any real behavior diverged (Principle X).
