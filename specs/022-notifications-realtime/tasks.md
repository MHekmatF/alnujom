...
- [ ] T014 [US5] Apply migrations T002–T010, T012, T013 in timestamp order via Supabase MCP `apply_migration` (memory `project_supabase_apply_via_mcp`); deploy `dispatch_push` (T011); register the three Vault secrets from env (only when push is being tested — omit to verify the degraded path); run `get_advisors` (no new RLS-disabled table; resolve any function-search-path warning); structural check (both tables RLS-on + writes REVOKEd; RPC grants; trigger present; `pg_publication_tables` shows the 3 adds). (Depends on T002–T013.) _(authored; orchestrator applies)_
