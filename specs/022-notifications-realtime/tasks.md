...
- [ ] T044 [US3] [US4] **⚠️ DEFERRED — human two-device QA (see DEFERRED.md).** Realtime publication on `listings`/`reports`/`user_roles` is live + the subscription/teardown/debounce logic passed the Opus review, but live delivery needs two devices. Two-device walk: admin counters (pending listings, reports) move ≤5 s with no refresh + reconcile after a forced network drop (SC-004); granting/revoking a role on device B updates device A's gated UI without re-login (SC-005); confirm the existing 3 observation points still work.
