...
Real deviations from `plan.md`/`data-model.md`, all intentional and committed:
...
- **13th migration `20260602120013_user_roles_replica_identity_full.sql`** — on-device QA found live role REVOKE didn't reach the client. `…010` set `user_roles` to `REPLICA IDENTITY DEFAULT`, but the `user_roles` Realtime subscription filters server-side on `user_id`; a DELETE/UPDATE WAL old-row at DEFAULT carries only the PK, so the filter never matches a revoke → the event is dropped (grants are INSERTs → full new row → delivered, so they worked). `…013` sets `REPLICA IDENTITY FULL`. **Reconcile R-190 / `…010`'s "DEFAULT suffices" comment** — it's wrong for a non-PK-filtered subscription. (listings/reports stay DEFAULT — their subscription is unfiltered.)
- **`AuthBloc` emit on permission change (`PermissionsChanged` event)** — the `user_roles` handler (T040) originally only called `_permissionChecker.refresh()`; the cache updated but no `AuthState` was emitted, so `BlocSelector`-gated UI never rebuilt. Fixed to refresh **then re-emit** the current profile's state (mirrors `_onAppResumedRefresh`). data-model T040 described "calls refresh()" — reconcile to "refresh() + re-emit so gated widgets rebuild".
...
**Live permission refresh (US4 / SC-005) — VERIFIED PASS after two fixes** (see §4): granting a role makes the gated Home admin entry appear live, and revoking removes it live, with NO re-login or navigation. Required `REPLICA IDENTITY FULL` on `user_roles` + the `AuthBloc` re-emit (both committed). The admin-counters half of T044 (SC-004) is still unverified — needs an admin login on the device.

**Two pre-existing emit-after-dispose bugs surfaced by fast navigation (NOT Phase 22 — fix later):**
- `AdSlotCubit.load` (`lib/features/ads/presentation/bloc/ad_slot_cubit.dart:41`) — `Bad state: Cannot emit new states after calling close` (Phase 21). Async ad load completes after the cubit is disposed (navigating away from an ad-slot screen mid-load). Guard with `if (isClosed) return;` before `emit`.
- `AgencyVerificationCubit.load` (`lib/features/agency/presentation/bloc/agency_verification_cubit.dart:101`) — same pattern (Phase 19). Same guard fix.
Both are caught by the Flutter error handler (logged, no crash), but they signal a recurring missing `isClosed` guard in async cubit loads — worth a sweep across the codebase.

**Device push (US2) — VERIFIED PASS live** (real Firebase `alnujom-real-estate-test`, 3 Vault secrets registered, app rebuilt with `google-services.json`): the app registered an FCM token on login; a server-side `enqueue_notification` → `pg_net` trigger → `dispatch_push` (200, ~1.2–2.3s) → FCM delivered the notification to the device; **tapping it deep-links correctly** (opened the A1 listing). Two findings:
- **(FIXED live) push-tap deep-link crashed with "No GoRouter found in context."** `NotificationPushListener` lives in `MaterialApp.router`'s `builder`, which is ABOVE the GoRouter InheritedWidget, so `context.go*` threw and the app fell back to Home. Fixed: `NotificationDeepLinkResolver.navigate()` now navigates via the DI router instance `getIt<GoRouter>()` (not the widget context), and the listener defers the background-tap nav to a post-frame callback. Re-verified on-device: tap → A1 listing. (In-app taps were always fine — that context is under the router.)
- **(fix-later, cosmetic) no heads-up banner.** The notification only appears in the pull-down shade, not as a pop-down heads-up banner, because the Android channel `alnujom_notifications` (referenced in `AndroidManifest.xml` `default_notification_channel_id`) is never CREATED with `IMPORTANCE_HIGH` — FCM logs `"Notification Channel ... has not been created by the app. Default value will be used."`. Notifications still deliver + are tappable + deep-link. Fix: create the channel at startup with `IMPORTANCE_HIGH` (native Kotlin in `MainActivity`/Application, or add `flutter_local_notifications`).

**Admin counters (US3 / SC-004) — VERIFIED PASS live:** the dashboard "Reports" badge ticked 0→1→0 via a backend report insert/delete, no refresh (debounced `DashboardCubit` refetch). Confirms unfiltered `listings`/`reports` subscriptions deliver on default replica identity. (Cosmetic, Phase 20, fix later: the admin-home **"Pending review" tile overflows by ~4px** with a 2-digit badge — `RenderFlex bottom overflowed by 4.0 pixels` — tighten the tile's vertical padding / allow the label to shrink.)
