...
- **`kPaletteTesterEnabled` (or equivalent) flag gating**: A build-time constant resolves to `true` in development and internal-QA builds, `false` in production builds, so the Palette Tester widget tree and its assets are tree-shaken from release artifacts.
