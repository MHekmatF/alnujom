...
- [ ] T002 [P] Create the `lib/features/admin/audit_logs/{data/{datasources,dtos,repositories},domain/{entities,repositories,usecases},presentation/{bloc,pages}}` folder skeleton, mirroring `lib/features/admin/reports/`.
...
- [ ] T024 [P] [US5] Add the US5 l10n keys (viewer title, column/field labels, empty-state; reuse the `denied=...` message pattern), namespaced `auditLogs*`, to BOTH `lib/l10n/app_ar.arb` and `lib/l10n/app_en.arb`; run `flutter gen-l10n`.
