...
- [ ] T018 [US3] **⚠️ PARTIAL —** Verify the data-driven reshape: grant `reports.manage` (or `audit_logs.view`) to a test user's role, refresh the `PermissionChecker` (re-login), reopen `/admin`, confirm the corresponding tile + counter now appear with no code change (SC-011, quickstart step 4b); confirm zero hardcoded role branches in `lib/features/admin/dashboard/` via grep (FR-015). — **Done**: the grep gate (FR-015) passed (zero hardcoded role branches; gating is 100% `PermissionKeys`/`PermissionChecker`), and the *data-driven reshape* is proven at the data layer (T017 showed the SAME RPC returns different counters purely as a function of the caller's permission set, zero code change). **Deferred**: the live UI walk — grant a permission → re-login → reopen `/admin` → tile+counter appear — requires a device/AVD session (see `DEFERRED.md`).
...
- [ ] T025 [P] **⚠️ PARTIAL —** Four-combination render pass on the reference Infinix Note 8 (≈480 dp) + a 412 dp emulator: cycle (light/dark) × (ar-RTL/en-LTR) across the grid, counters, coming-soon tiles, loading/empty-zero/error states, and the audit viewer; confirm all strings localized, direction-correct, Phase 2 tokens only (no inline hex/font/padding). (SC-007, FR-017) — **Partial**: the static layer is verified — `flutter analyze --fatal-infos` clean; the UI audit confirmed Phase 2 tokens (one minor `size: 32`/badge-padding nit logged to the UI backlog, not a token violation of the gate); existing `ThemeGallery` widget tests render light/dark × ar/en. **Deferred**: the on-device/AVD four-combo *visual* walk of the new dashboard grid + audit viewer (see `DEFERRED.md`).
...
- [ ] T027 **⚠️ PARTIAL —** Execute the full `quickstart.md` (steps 1–11) end-to-end and record pass/fail per Success Criterion in the run-log; file any gap in `specs/020-admin-dashboard/DEFERRED.md` (memory `feedback_strict_task_completion`). — **Done autonomously**: step 1 (migrations applied + advisors, T006), step 8 wire-gate (T017), the FR-015 grep + constitution gates (T026), build/analyze/test green. `DEFERRED.md` created. **Deferred**: the device-driven steps (tile-set-by-role walk, counter-equality, quick-action navigation, audit-viewer pagination/read-only, four-combo render) — see `DEFERRED.md`.
