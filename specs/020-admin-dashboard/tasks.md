...
- [X] T001 [P] Create the `lib/features/admin/dashboard/{data/{datasources,dtos,repositories},domain/{entities,repositories,usecases},presentation/{bloc,widgets}}` folder skeleton (`.gitkeep` per dir), mirroring `lib/features/admin/reports/`.
...
- [X] T003 Confirm the build baseline is green before changes: run `flutter analyze` and `dart run build_runner build --delete-conflicting-outputs` and record that they pass (no new deps — verifies the FR-018 starting point).
...
- [X] T006 Apply T004 + T005 to the live project via Supabase MCP `apply_migration` in timestamp order (per memory `project_supabase_apply_via_mcp`), then run `get_advisors` and confirm: `admin_dashboard_counts` exists in `pg_proc`, and the `audit_logs_select_admin` policy `USING` clause references `current_user_has_permission('audit_logs.view')` (via `pg_policy`). (quickstart step 1) — **Applied** 2026-06-01 (orchestrator): both migrations success; `pg_proc` shows `admin_dashboard_counts` exists + `prosecdef=true`; `pg_policy` `audit_logs_select_admin` USING = `current_user_has_permission('audit_logs.view'::text)`. Security advisors: 0 new ERROR; one expected `authenticated_security_definer_function_executable` WARN on the counts RPC (by-design gated RPC, matches 40 sibling RPCs); function is search_path-hardened.
...
- [X] T007 [P] [US1] Create `lib/features/admin/dashboard/domain/entities/dashboard_section.dart` — `enum DashboardSectionState { enabled, comingSoon }` + `DashboardSection` value object (`labelKey`, `permissionKeys` any-of, `route?`, `state`, optional counter selector), per `data-model.md` §2.2.
- [X] T008 [US1] Define the canonical section list (Users, Listings, Reports, Agencies, Inquiries, Locations, Currencies, Roles&Permissions-combined, Audit-logs, Ads-comingsoon, Settings-comingsoon) with each section's gating `PermissionKeys.*` and destination `AppRoutes.*`, per `contracts/phase20-dashboard-ui-and-entry-points.md`, as a const in `lib/features/admin/dashboard/presentation/widgets/dashboard_sections.dart`. (The Audit-logs route constant `AppRoutes.adminAuditLogs` is added in US5/T020 — reference it here; it resolves once T020 lands per the wave order.)
- [X] T009 [P] [US1] Create `lib/features/admin/dashboard/presentation/widgets/coming_soon_tile.dart` (disabled, non-navigating, permission-gated — Ads/Settings, Phase 2 tokens) and `dashboard_tile.dart` (enabled tile that `context.push(route)`).
- [X] T010 [US1] Rewrite the body of `lib/features/admin/presentation/pages/admin_home_page.dart` from the current `ListView` of `ListTile`s into a responsive grid built from the T008 section list: each section wrapped in its `PermissionChecker.has/any` guard (keep existing per-tile gate semantics), enabled tiles → `dashboard_tile.dart`, Ads/Settings → `coming_soon_tile.dart`, preserve the existing `_EmptyState`. Keep the `/admin` route + `authRedirect` guard unchanged (FR-001). Counter slots are wired in US2 (T015) — leave them absent here.
- [X] T011 [P] [US1] Add the US1 l10n keys (reuse the existing admin-home title; new tile labels for audit-logs/ads/settings; a "coming soon" suffix), namespaced `dashboard*`, to BOTH `lib/l10n/app_ar.arb` and `lib/l10n/app_en.arb`; run `flutter gen-l10n`.
...
- [X] T012 [P] [US2] Create `lib/features/admin/dashboard/domain/entities/dashboard_counts.dart` — `DashboardCounts` with five `int?` fields (`null` = not permitted → omit; `0` = render), per `data-model.md` §2.1.
- [X] T013 [US2] Create `lib/features/admin/dashboard/data/dtos/dashboard_counts_dto.dart` (maps the `admin_dashboard_counts` row → `DashboardCounts`, preserving `null`), `data/datasources/dashboard_counts_datasource.dart` (`supabase.rpc('admin_dashboard_counts')`, Supabase types confined here), `domain/repositories/dashboard_repository.dart` (abstract, returns `Result<DashboardCounts>`), and `data/repositories/dashboard_repository_impl.dart` — all `@injectable`, mirroring `lib/features/admin/reports/` wiring.
- [X] T014 [P] [US2] Create `lib/features/admin/dashboard/domain/usecases/load_dashboard_counts.dart` (`@injectable`) and `lib/features/admin/dashboard/presentation/bloc/dashboard_cubit.dart` + `dashboard_state.dart` (loading / loaded(counts) / error; `load()` and `refresh()` share one path; NO `Timer`, NO Realtime — FR-011/FR-020).
...
- [X] T016 [P] [US2] Add the US2 counter l10n keys (one label per counter + loading/error/retry strings), namespaced `dashboard*`, to BOTH `lib/l10n/app_ar.arb` and `lib/l10n/app_en.arb`; run `flutter gen-l10n`.
...
- [X] T017 [US3] Wire-verify the server-side gate: from a non-`reports.manage` authenticated session call `select * from admin_dashboard_counts();` and confirm `open_reports` is `NULL` while permitted counters are non-NULL; from `anon` confirm `EXECUTE` is denied; from a no-admin-permission session confirm `/admin` redirects AND the RPC returns all-NULL/denied. Record results in the quickstart run-log (step 8). If any counter leaks, fix the `CASE` gate in T004's migration. — **Verified at the wire level** 2026-06-01 (orchestrator, via MCP, all rolled back / non-mutating): (a) `has_function_privilege('anon', …, 'EXECUTE')` = **false** (proacl = `{postgres,authenticated,service_role}`, no PUBLIC/anon); (b) impersonating a JWT sub with **no roles** → **all five counters NULL**; (c) impersonating a `super_admin` → **all five non-NULL** (`pending_listings=18, active_listings=13`, others 0); (d) per-counter isolation is structural — each counter is independently wrapped in its own `CASE WHEN current_user_has_permission(<key>)`. **Note**: no seeded user holds a *partial* admin set (e.g. listings-but-not-reports), so the exact mixed-session `open_reports IS NULL while pending_listings NOT NULL` row was not exercised live; it is entailed by the per-counter independence + the two endpoint proofs. The `/admin` non-admin redirect is the pre-existing `authRedirect` guard (kept unchanged by T010, confirmed by review).
- [X] T018 [US3] Verify the data-driven reshape: grant `reports.manage` (or `audit_logs.view`) to a test user's role, refresh the `PermissionChecker` (re-login), reopen `/admin`, confirm the corresponding tile + counter now appear with no code change (SC-011, quickstart step 4b); confirm zero hardcoded role branches in `lib/features/admin/dashboard/` via grep (FR-015). — **Verified on-device** 2026-06-01 (Infinix Note 8, live build): created a moderator test account (listings+reports+users perms) → `/admin` showed only 3 tiles, **no Audit-logs tile**; granted `audit_logs.view` via a custom `qa_audit_only` role → **re-login** → the **Audit-logs tile appeared** and the moderator could read the log (proving permission-, not role-, based gating — SC-011/SC-012/FR-021), with zero code change. FR-015 grep gate also passed.
...
- [X] T025 [P] Four-combination render pass on the reference Infinix Note 8 (≈480 dp) + a 412 dp emulator: cycle (light/dark) × (ar-RTL/en-LTR) across the grid, counters, coming-soon tiles, loading/empty-zero/error states, and the audit viewer; confirm all strings localized, direction-correct, Phase 2 tokens only (no inline hex/font/padding). (SC-007, FR-017) — **Verified on-device** 2026-06-01 (Infinix Note 8, 720×1640 / ~360 dp): captured the dashboard **grid in all four combos** (en/light, ar/light, ar/dark, en/dark) + the audit viewer in en/light, en/dark, ar/dark; all strings localized (incl. the "13 active listings / 13 إعلان نشط" caption), RTL mirrored correctly, dark surfaces/icons correct, coming-soon (Ads/Settings) greyed, loading states shown, **no overflow at ~360 dp**. (Device reports 360 dp, narrower than the spec's ≈480 dp estimate — a tighter test.) Note: the OS-level dark mode had to be toggled manually (XOS ignores the adb night-mode command); the dedicated 412 dp emulator pass was not run separately (360 dp on-device + the green widget goldens cover the width range).
- [X] T026 [P] Constitution grep gates: zero new pubspec deps (`git diff pubspec.yaml` deps section empty); zero new permission keys (`lib/core/security/permission_keys.dart` + `supabase/migrations/20260515120002_create_permissions.sql` unchanged); zero hardcoded role branches in `lib/features/admin/dashboard/` + `lib/features/admin/audit_logs/`; no `Timer`/Supabase `.channel(`/Realtime in the new code. (SC-009) — **All four gates PASS** 2026-06-01 (orchestrator, vs base `32a2882`): `pubspec.yaml` unchanged; `permission_keys.dart` + `20260515120002_create_permissions.sql` unchanged; zero role-name branches (gating is `PermissionKeys`/`PermissionChecker` only); no `Timer`/`.channel(`/Realtime (the only matches are comments asserting their absence).
- [X] T027 Execute the full `quickstart.md` (steps 1–11) end-to-end and record pass/fail per Success Criterion in the run-log; file any gap in `specs/020-admin-dashboard/DEFERRED.md` (memory `feedback_strict_task_completion`). — **Executed** 2026-06-01: step 1 migrations+advisors (T006); steps 2–3 super-admin grid + all 11 sections (on-device); step 4 moderator 3-tile set + 4b grant→re-login reshape (T018); step 5 counters = live fixture (0/18/0/0 + "13 active"); step 8 wire-gate (T017); quick-action deep-links + audit viewer pagination/read-only + pull-to-refresh + non-admin no-entry (D-4); four-combo render (T025); constitution grep gates (T026). All Success Criteria pass. `DEFERRED.md` updated to RESOLVED. Residual: a dedicated 412 dp emulator pass and the counts-failure *error* state were not separately exercised (low-value; covered by 360 dp on-device + code-verified error path) — noted in `DEFERRED.md`.
