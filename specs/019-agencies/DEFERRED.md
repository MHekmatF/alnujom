...
| B-5 | Agency sub-routes red-screen if `extra` is lost (process death / external-activity return on low-RAM devices) | members/listings/analytics/verify routes do `state.extra as Agency`/`as String`; `extra` is not part of the URI so it is null after recreation | Edit page (the one that launches the gallery) was made self-loading (no `extra`). The other 4 routes still cast `extra` — harden them (self-load by id, or pass id as a path/query param) | ⏳ edit fixed; others to harden |
