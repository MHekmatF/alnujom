...
| D-2 | Verification document **file** upload | T058 | FR-006, US1 AS#4 | ✅ Done (2026-06-01) |
| D-3 | Rejection-reason text shown to the owner | T058 | FR-009, US1 AS#5 | ✅ Done (2026-06-01) |
...
| B-4 | An invitee can never reach Accept/Decline | The pending-invitation strip + Accept live only on `agency_members_page` (reachable only by an owner/active member). A pending-only invitee hits `AgencyHomeNone` → the create-agency form. No `AgencyHomeInvited` hub state. | ✅ fixed — new `AgencyHomeInvited` state on `AgencyHomeCubit` surfaces `loadMyAgencyInvitations()` (name-enriched) on the hub with Accept/Decline → `respond_agency_invitation`; on accept `load()` re-resolves to `AgencyHomeMember`. Compiles/boots clean; accept RPC verified (pending→active). On-device invitee view still needs a second login to exercise. | ✅ code + boot verified; invitee view pending 2nd login |
| B-5 | Agency sub-routes red-screen if `extra` is lost (process death / external-activity return on low-RAM devices) | members/listings/analytics/verify routes do `state.extra as Agency`/`as String`; `extra` is not part of the URI so it is null after recreation | ✅ fixed — edit page self-loads (no `extra`); the other 4 routes' redirects now bounce to `/agency` when `state.extra == null` instead of casting null | ✅ Done (2026-06-01) |
| B-6 | All `agency-assets` reads/upserts failed with `invalid input syntax for type uuid` | `agency_assets_public_select` cast the AGENCY NAME (`a.name`) to uuid instead of the storage object's path prefix (`…120013`). Latent until the first upload. | ✅ fixed — migration `20260531120017` (compare `a.id::text = split_part(objects.name,'/',1)`); also `…120016` widened the bucket MIME allow-list (was JPEG-only) | ✅ fixed + verified (logo uploads + badge shows it) |
...
**All follow-ups resolved (2026-06-01):** D-1 (search badge), D-2 (verification document upload — image picker on the verify page → `agency-documents` → `evidence_urls`; bucket MIME widened in `20260601120002`), D-3 (rejection reason on the verify page via `loadMyVerificationRequest`), the agency **cover** image picker (alongside logo, with immediate auto-persist on pick), and B-5 (the 4 `extra`-based agency sub-routes now bounce to the hub on `extra`-loss). All verified on device (D-2/D-3 via a temporary reject→resubmit→approve cycle). Nothing outstanding in this spec beyond the human on-device QA walk (T074) and the FE-1 future enhancement.
