...
- [ ] T074 **⚠️ PARTIAL / NOT RUN —** **Device QA** (Infinix Note 8 + Pixel 8 Pro AVD): apply the 14 migrations (`…001`–`…014`) + deploy the `moderate_agency` Edge Function, then walk the full `quickstart.md` recipe on-device + at the wire level (all 15 SCs). **NOT done by the orchestrator** — the live Supabase apply must use the exact reviewed migration files with DB credentials (62 KB of DDL; `supabase db push` mis-diffs against the MCP-applied Phase 16–18 history, and hand-transcription onto a live production DB is unsafe), and the on-device UI walk requires a physical device/emulator the orchestrator cannot drive. **Runbook** for the human in the wave-close report; this is the one remaining gate before the spec is operationally "done" per DoD §11.
