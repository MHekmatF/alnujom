...
- [ ] T058 [US1] **⚠️ PARTIAL —** Create `lib/features/agency/presentation/pages/agency_verification_page.dart` — doc upload + ID/registration fields + status banner + rejection reason. **Shipped**: ID/registration fields + submit + status banner. **Deferred** (see `DEFERRED.md` D-2, D-3): document **file** upload (the repo's `submitVerification` accepts pre-uploaded `evidenceUrls` but no UI upload path is exposed) and the **rejection-reason text** display (no `loadMyVerificationRequest` read exists; only `agency.status` is shown).
...
- [ ] T060 [US5] **⚠️ PARTIAL —** Add an OPTIONAL `agencyName`/`agencyLogoPath` (or `Widget? agencyBadge`) param to `lib/core/widgets/property_card.dart` and render it only when non-null — NO layout change when null (FR-023). Card sites pass the badge fields from `v_listings_public`. **Shipped**: the `PropertyCard` param + the **home-feed** card badge (home query amended to embed the agency). **Deferred** (see `DEFERRED.md` D-1): the **search-results** badge — the `search_listings` RPC does not return agency fields, so search cards have no badge until that RPC + the search DTO are extended.
...
- [ ] T074 **⚠️ PARTIAL —** **Backend applied + verified; on-device UI walk pending.** **Done (orchestrator, via Supabase MCP on project `hczsgceagommznjaohyk`):** all 15 migrations applied in order (`…001`–`…014` + the `…015` `moderate_agency_internal` OUT-param-ambiguity fix found at apply time); the `moderate_agency` Edge Function deployed (v1, `verify_jwt=true`); structural verification (3 tables, 3 `v_listings_public` badge columns, `v_agencies`, 12 functions, 2 buckets, the listings FK, 4 audit triggers — all present); `get_advisors(security)` shows only the expected/accepted `security_definer_view` + `*_security_definer_function_executable` notices (0 RLS-disabled); and an end-to-end moderation smoke test (`moderate_agency_internal('approve')`) passed and rolled back (agency→approved, request→approved, reviewed_by set; the ambiguity fix verified). **Pending (needs a device — orchestrator cannot drive the Flutter app):** the on-device `quickstart.md` walk + the per-auth-role wire-level read-matrix (SC-009 as anon/owner/member/admin) + the 4-combination theme×locale render (SC-013) on the Infinix Note 8 / Pixel 8 Pro AVD.
- [X] T075 Run the constitution grep gates (SC-014) — **ALL PASS**: `pubspec.yaml`/`pubspec.lock` unchanged vs `origin/main` (zero new deps); zero hardcoded role/permission string branch in `lib/features/agency/` + `lib/features/admin/agencies/` (gating is `is_agency_admin` / `PermissionKeys.agencies*`); no `package:supabase_flutter` under any agency `domain/`/`presentation/`; no inline `Text('…')` literal in the agency feature widgets; no Phase 19 migration adds a value to the `listings.status` CHECK or touches `lead_events`.
- [X] T076 Contract audit — **PASS**: all 8 `contracts/*.md` exist, each carries a Smoke-tests section, and the `quickstart.md` recipe covers their subjects (create-agency, members, verification, `v_agencies`/policies, write RPCs, `moderate_agency`, `submit_listing` amendment, UI/entry-points). The smoke-test *execution* itself runs as part of T074 device QA (wire-level + on-device); no uncovered contract assertion found.
