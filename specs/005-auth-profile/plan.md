...
- **6 new SQL migration files** under `supabase/migrations/` named with synthetic-monotonic 14-digit timestamps `20260510120001` through `20260510120006`, ordered after Phase 4's `20260506120006_enable_vault.sql` (R-01). Migrations 1–5 implement the FR deliverables; migration 6 was added during implementation to close `get_advisors` warnings introduced by the SECURITY DEFINER helpers in 3/4/5 (search_path + anon-executable hardening).
...
│   └── 20260510120006_phase5_advisor_hardening.sql       # NEW — advisor hardening added during implementation (R-01 addendum). Re-creates current_user_is_admin() with explicit SET search_path = public (the body swap in 20260510120003 declared LANGUAGE SQL STABLE without an explicit search_path, which the advisor flags as mutable). REVOKE EXECUTE … FROM PUBLIC, anon on the seven Phase-5 SECURITY DEFINER functions; auto_create_account_approval_request also revoked from authenticated (trigger-only). Explicit GRANT EXECUTE … TO authenticated on the six user-callable RPCs. Constitution III defense-in-depth; preserves the R-12 central-helper invariant — touches one function definition, no policy file edited.
...
**Structure Decision**: Phase 5's footprint is split between (a) a small backend extension on top of Phase 4 (6 new migrations — 5 FR-deliverable + 1 advisor-hardening, 1 new policy file, 1 new Edge Function, 1 new doc file, 2 doc-file updates) and (b) the first real `lib/features/` folders on the Flutter side (4 feature folders — auth, profile, onboarding, admin/account_approvals — each with the full Clean Architecture trio). The "edits to existing Phase 4 artifacts" are: the appends to `supabase/docs/profiles.md` + `supabase/docs/audit_logs.md` (text-only), the addition of the `isAdmin` field to the existing Phase 4 `Profile` entity (additive — does not break Phase 4's contract), the `app.dart` wiring that registers the new top-level providers and routes, **and one Dart file: `lib/core/errors/failure.dart`'s base class is loosened from `sealed class Failure` to `abstract class Failure` so feature-typed failure hierarchies (`AuthFailure`, `ProfileFailure`) can extend it and slot into `Result<T>` (research R-22)**. The four existing `final class` Phase-4 failures retain their bodies and `final` modifiers; no Phase 4 call site is affected. No Phase 4 policy file or migration body is edited (the central-helper R-05 invariant is preserved — `current_user_is_admin()` is replaced by `CREATE OR REPLACE FUNCTION`, not by editing policies). Constitution IX is enforced by the directory layout: every `domain/` subfolder is Supabase-free; the only files importing `package:supabase_flutter` are the four `data/datasources/*.dart` files plus the existing `supabase_client_wrapper_impl.dart`. Migrations are applied to the **remote** Supabase project via Supabase MCP `apply_migration`; the Edge Function is deployed via Supabase MCP `deploy_edge_function`. There is no local Supabase setup in this phase (per Phase 4's R-01).
