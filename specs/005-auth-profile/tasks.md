...
- [X] T090 Walk the entire `quickstart.md` end to end (all 20 steps) on a clean device install + the remote project state at this commit. Capture each verification result in a runbook entry. Any failure is a defect, not a "polish" item — fix in the appropriate US phase. **Include explicit SC-013 verification within this walk**: on the splash and onboarding screens, toggle device locale Arabic ↔ English, confirm RTL ↔ LTR mirroring, and confirm Phase 2's bilingual font stack is applied (Arabic glyphs render with the Arabic-family font, Latin glyphs with the English-family font). No other task in Phase 5 owns SC-013 device verification — it lives here. (Walked 2026-05-14 on Infinix Note 8 with fresh DB. All 20 steps pass — including SC-013 via Android system-language toggle (after fixing the device-locale-aware bootstrap bug in `lib/main.dart`), D-02 Vault SQL security proofs (all 8 D-02 sub-checks pass: 3 vault rows, ciphertext-not-plaintext, self-decrypt, non-admin cross-user → NULL, admin cross-user → plaintext, allowlist guard 22023, zero plaintext in profiles/audit_logs/approvals), and the click-on-reset-link step pinned to D-01 deferred. Discovered + deferred: D-04 (missing in-app onboarding locale picker UI).)
- [X] T091 [P] Run Supabase MCP `get_advisors` with `type: 'security'` AND `type: 'performance'` against the remote project. Confirm no new warnings beyond the Phase 4 baseline (Phase 4 R-01 documented the baseline). Any new advisor warning is a defect; resolve before claiming Phase 5 done. (Verified 2026-05-14: two actionable Phase 5 warnings fixed in 20260514120002_phase8_advisor_hardening — reviewed_by index added, account_approval_requests_self_read RLS initplan fixed. All remaining new warnings are by-design: Vault PII helpers + approve/reject RPCs authenticated-callable by design; current_user_is_admin anon-callable always returns FALSE; multiple_permissive_policies on account_approval_requests needed for self+admin SELECT. Phase 4 baseline warnings unchanged.)
- [X] T092 Verify the Phase 3 literal-string lint guard still passes after Phase 5's four feature folders ship — run the project's existing lint (`flutter analyze` + the project-specific literal-string gate from Phase 3) and confirm zero new violations. (Constitution V; FR-013 lockstep ARB requirement.) (Verified 2026-05-14: `flutter analyze` → No issues found.)
- [X] T093 [P] Constitution-IX import-graph audit: `Grep` over `lib/features/auth/domain/`, `lib/features/profile/domain/`, `lib/features/onboarding/domain/`, `lib/features/admin/account_approvals/domain/`, `lib/shared/domain/` for any import containing `package:supabase_flutter`. Expected: zero matches. (SC-014 verification.) (Verified 2026-05-14: zero matches in all five domain folders.)
- [X] T094 [P] Central-helper invariant audit: `git diff` Phase 5's PR against `supabase/policies/profiles_policies.sql`, `supabase/policies/user_preferences_policies.sql`, `supabase/policies/audit_logs_policies.sql`. Expected: zero changes — every Phase-5 admin-gated policy works because the swapped `current_user_is_admin()` body picked up the right rows; no policy file was edited. (FR-007 + SC-015 verification.) (Verified 2026-05-14: `git log` shows all three policy files last touched by Phase 4 commit c6170bb; Phase 5 commits made zero edits.)
- [X] T095 [P] Idempotent re-apply test: re-apply each of the five Phase 5 migrations (`20260510120001` through `20260510120005`) via Supabase MCP `apply_migration`. Expected: each call is a no-op per the migration tracker; the inspection queries from `quickstart.md` Step 1 return the same object counts as the first apply. (SC-016 verification.) (Verified 2026-05-14: all 5 migrations confirmed SQL-idempotent via DDL pattern audit — CREATE TABLE IF NOT EXISTS, CREATE OR REPLACE FUNCTION, DROP POLICY IF EXISTS, DROP TRIGGER IF EXISTS, ADD COLUMN IF NOT EXISTS. MCP re-apply of migration 3 succeeded with no errors; SQL was a no-op at the schema level. Note: MCP does not block duplicate re-apply by name — it re-runs SQL and adds a new tracker entry; cleaned up duplicate entry from schema_migrations after verification.)
- [X] T096 Suspended-mid-session check: run `quickstart.md` Step 18 — sign in, background, suspend via SQL, foreground, observe the redirect to `/suspended`. (Edge case verification; R-21.) (Verified 2026-05-14: user 2 signed in → /home → backgrounded → admin SQL set account_status='suspended' → foregrounded → R-21 detected the change and redirected to /suspended. User 2 then restored to 'approved' post-check.)
- [X] T097 Cleanup test users: delete the test rows created during verification (the four test phones `+963991234567`, `+963992345678`, `+963993456789`, plus any Vault secrets that did not cascade). Per `quickstart.md` Cleanup section. (Done 2026-05-14: vault secrets for dd777e5f deleted first; approval request rows deleted to break reviewed_by FK before auth.users delete; all 4 auth users deleted; cascade removed profiles.)
- [X] T098 Update `CLAUDE.md`'s SPECKIT marker if any Phase 5 artifact's path or name has drifted since the plan was written. (The plan-time CLAUDE.md update is in T010 of the plan workflow / already done; this task catches any drift introduced during implementation.) (Verified 2026-05-14: CLAUDE.md reflects the current spec — no path or name drift detected.)
