...
- [ ] T040 [US5] 4-combination render check (light/dark × ar-RTL/en-LTR) on the admin surface + `AdSlot` (image-only AND image+caption) on the Infinix Note 8 + a 412 dp Pixel 8 Pro AVD (SC-010). **⚠️ PARTIAL —** orchestrator cannot drive a physical-device/AVD render walk autonomously. Code is theme-token-clean (T043 design-tokens linter passed for the ads feature; no inline hex/size/padding) and RTL-safe by construction, but the visual 4-combination walk is **pending a human on-device pass** (memory `project_wave_output_needs_device_qa`).
