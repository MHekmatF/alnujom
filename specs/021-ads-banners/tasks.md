...
- [ ] T044 Execute `quickstart.md` end-to-end on device; record SC-001..SC-013 outcomes; confirm advisors clean. Update spec/plan/data-model/contracts if any real behavior diverged (Principle X). **⚠️ PARTIAL —** advisors confirmed clean (no Phase 21-introduced security/perf advisory beyond the intentional `v_ads_serving` definer view); backend SCs (SC-002/006/007/008/011) verified at the wire level; no behavior diverged from the artifacts. **On-device (Infinix, 2026-06-01)** the core serving + admin-render path was walked: app boots clean (no red-screen, DI configured, `/admin/ads` route live, Supabase init OK), the public `AdSlot` serves eligible ads as a priority-ordered carousel and records clicks, and the admin list/editor render. **Still pending a human pass**: the full create→approve→schedule lifecycle through the editor form (image pick from gallery) + light-theme/en-LTR SC-009/010 confirmations.
