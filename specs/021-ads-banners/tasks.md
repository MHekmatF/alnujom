...
- [X] T040 [US5] 4-combination render check (light/dark × ar-RTL/en-LTR) on the admin surface + `AdSlot` (image-only AND image+caption) on the Infinix Note 8 + a 412 dp Pixel 8 Pro AVD (SC-010). **⚠️ PARTIAL —** **dark × ar-RTL fully walked on the real Infinix Note 8 (2026-06-01)**: admin dashboard Ads-tile flip (enabled, not greyed), `AdsListPage` (status chips/toggles/archive/FAB), `AdEditorPage` (title/image-picker/both-or-neither captions/link-type dropdown/schedule), and the public `AdSlot` carousel (image+caption, auto-advance, swipe, page indicator) all render correctly. **This walk found + fixed a real bug**: the carousel `PageView` had no bounded height in a `SliverToBoxAdapter` → invisible; fixed by wrapping in `AspectRatio(16/5)` (commit `528dc17`). **User on-device pass (2026-06-01) confirmed** the light-theme + en-LTR combinations on the Infinix. (412 dp Pixel 8 Pro AVD pass optional — Infinix is the gating device per `feedback_avd_acceptable_qa`.)
- [X] T041 [US4] Wire-level security checks: from a non-`ads.manage` (and anon) session, confirm `ads`/`ad_placements` insert-update-delete, `ads`-bucket write, `ad_impressions` insert, and the admin RPCs are all DENIED; anon `v_ads_serving` returns eligible-only (no drafts/inactive/expired/archived, no admin fields) — SC-008. — **DONE** (live MCP role-simulation): anon `SET LOCAL ROLE` probe → `v_ads_serving`=1 eligible row, inactive ad excluded, base `ads`/`ad_impressions`=0 rows (admin-only RLS); anon `INSERT ads` denied (insufficient_privilege); table grants show NO insert/update/delete for anon/authenticated (matches `agencies`/`reports` sibling posture; the `TRUNCATE` grant is runtime-denied); RPC grants = 4 writers authenticated-only + `record_ad_event` anon+authenticated, none to PUBLIC.
- [X] T042 [US3] Click verification: `SELECT COUNT(*) FROM ad_impressions WHERE ad_id='…' AND kind='click'` == taps; `SELECT COUNT(*) … WHERE kind<>'click'` == 0; offline tap still opens target (SC-006/007); `SELECT … FROM audit_logs WHERE target_type='ads'` shows create+delete (SC-011). **⚠️ PARTIAL —** wire-level VERIFIED: anon `record_ad_event` on an eligible ad → `clicks=1, non_clicks=0` (clicks-only); inactive-ad click rejected `ad_not_eligible`/23514; `audit_logs` `ad.created`/`ad.activation_changed`/`ad.deleted` (target_type='ads') confirmed. **On-device VERIFIED (Infinix, 2026-06-01)**: 2 real taps on the home `AdSlot` banner → exactly 2 `ad_impressions` rows, all `kind='click'`, zero non-click (clicks == taps, clicks-only); external-link tap opened the target in Chrome AND still recorded the click. (Offline/airplane-mode tap is best-effort by design — FR-017: the target still opens, the click is silently dropped; not separately exercised, non-gating.)
- [X] T043 Run the full CI linter suite (`flutter analyze` + format + design-tokens + l10n-parity + l10n-literals + SDK-boundary — memory `project_wave_run_full_verify_suite`); grep the ads feature for hardcoded role branches (none — FR-022); confirm `git diff pubspec.yaml` empty + no ad-network dep + no §9.1/seed change (SC-013). — **DONE**: analyze + dart format + the 3 `tool/` linters + SDK-boundary all clean for the ads feature (fixed 5 design-token + 2 curly-brace + format issues the sub-agents missed; commit `f5ceeda`); no hardcoded role branch; `git diff pubspec.yaml/.lock` = 0 lines; no ad-network dep; no §9.1/role-seed migration. (Pre-existing Phase 19 agency lint debt unchanged — not Phase 21.)
- [X] T044 Execute `quickstart.md` end-to-end on device; record SC-001..SC-013 outcomes; confirm advisors clean. Update spec/plan/data-model/contracts if any real behavior diverged (Principle X). **⚠️ PARTIAL —** advisors confirmed clean (no Phase 21-introduced security/perf advisory beyond the intentional `v_ads_serving` definer view); backend SCs (SC-002/006/007/008/011) verified at the wire level; no behavior diverged from the artifacts. **On-device (Infinix, 2026-06-01)** the core serving + admin-render path was walked: app boots clean (no red-screen, DI configured, `/admin/ads` route live, Supabase init OK), the public `AdSlot` serves eligible ads as a priority-ordered carousel and records clicks, and the admin list/editor render. **User on-device pass (2026-06-01)** completed the create-an-ad flow (editor form + gallery image pick + placement + save → ad served on Home + tap-through opened the target) and the light/en SC-009/010 glance. Phase 21 acknowledged complete by the user.
