-- Phase 18 (spec/018-reports-moderation) — clear the review soft-claim on resolve.
--
-- QA cosmetic finding: resolved/dismissed cards still showed "Being reviewed
-- by …" (present tense) because reviewing_by / reviewing_started_at were left
-- set after resolution. A terminal report is no longer "being reviewed", so the
-- resolution now clears the claim (the UI hides the line when reviewing_by is
-- null). Body is otherwise identical to 20260530120010.
