...
void main() {
  ...
  // RTL/LTR matrix — T062
  testWidgets('AppBottomNav tab order under RTL: الرئيسية (home) is rightmost, '
      'حسابي (profile) is leftmost', (tester) async {
    await pumpWidgetKit(
      tester,
      AppBottomNav(currentIndex: 0, onTabSelected: (_) {}, onAddPressed: () {}),
    );
  ...
  testWidgets('AppBottomNav tab order under LTR: الرئيسية (home) is leftmost, '
      'حسابي (profile) is rightmost', (tester) async {
    await pumpWidgetKit(
      tester,
      AppBottomNav(currentIndex: 0, onTabSelected: (_) {}, onAddPressed: () {}),
