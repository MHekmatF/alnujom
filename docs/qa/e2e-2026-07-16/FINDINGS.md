...
- **Fix:** add the ARB entry (ar + en) for `listing_media.images_below_minimum` (e.g. "الرجاء إضافة صورة واحدة على الأقل") and map the validation code to it. Audit the other server/RPC validation codes for the same missing-translation gap.
...
- **Likely cause:** the send-success path doesn't append to the local message list / doesn't `emit` a new state, and/or the Realtime subscription for the thread isn't updating the UI on own-INSERT (matches the project's documented "cache refresh must emit a state or a BlocSelector-gated UI won't rebuild" gotcha). **Fix:** optimistically append the sent message on send-success (and reconcile with the Realtime INSERT), so the thread updates immediately.
