...
| Item | Status | Notes |
|---|---|---|
| **SEC-I1 coordinate leak** | ✅ Anon vector closed | Detail page now sources the marker from the gated `v_listings_map_public`; anon SELECT on `listings.latitude/longitude` revoked (`…120007`), verified. **Residual (raw-API-abuse only):** the app's own UI never exposes coords to a regular user (home/search use the coordless view, detail uses the gated view, owners see only their own) — but a signed-up user could make a RAW REST call to read others' coords off the base table. Fully closing that means revoking the columns from `authenticated` too, which breaks 5 owner/admin read sites (3 edit-form `.select()`, the admin revision snapshot, the admin `SELECT *` preview) → needs an owner/admin coordinate RPC + converting those 5 sites + edit/moderation-flow testing. Deferred as a focused, tested change; the severe no-login bulk-scrape is closed. |
| **Item 18 — RLS `auth.uid()` wrap** | ✅ Done (ALL tables) | Wrapped `auth.uid()` → `(select auth.uid())` across every public table — hot feed/search/chat (`…120008`) + the remaining ~19 tables (`…120009`), generated + reviewed + verified (0 unwrapped remain anywhere; semantically identical). Residual (higher risk / near-zero benefit at current scale): wrapping `current_user_has_permission()` + merging the 27 duplicate permissive policies. |
| **Item 17 — search index** | ⏸ Deferred (deeper finding) | The "obvious" fix — fold `listing_details.description` into the `search_vector` GIN — would **regress Arabic search**: `search_vector` uses the `'simple'` config (whole-word, no stemming), so folding description in changes it from substring (ILIKE matches "شقة" inside "الشقة" — the definite article is everywhere in Arabic) to whole-word-only. The non-regressing fix is `pg_trgm` + a trigram GIN + a UNION restructure of `search_listings`/`search_map` (which complicates cursor pagination). Zero benefit at 26 listings; needs a search-quality decision + real-data `EXPLAIN`. Left as a focused, tested change. |
| **Item 15 — home virtualization** | ✅ Done + AVD-verified | Converted to `CustomScrollView` + `SliverList.builder` (the feed no longer keeps every paged card + image alive). Preserved the Blue Crown `-18px` sheet-overlap by moving the feed OUT of the sheet Container and setting the scaffold bg to `card` (so the overlap + feed rows read seamlessly). Verified on the Pixel 8 Pro (guest home, Arabic): crown, rounded sheet overlap, category grid, featured rail, ad and the scrolling feed all render identically to before — no seam, real photos, smooth scroll. |
| **Item 19 — APK shrink** | ✅ Done (finding: use ABI split, not minify) | R8 minify was validated (release build passes with `proguard-rules.pro`) but **left disabled**: on a Flutter APK it shrank ~0.5 MB (90 → 89.5) — the bulk is native `.so`, which R8 doesn't touch — for release-only runtime risk. The real win, no code change: `flutter build apk --release --split-per-abi --dart-define-from-file=.env.json` → **verified 2026-07-17: arm64 APK 37.8 MB (was 90 MB fat) — a 58% smaller download**; armeabi-v7a 36.0 MB, x86_64 39.1 MB. Font prune (~1.5 MB) already landed. Config left ready-to-enable in `build.gradle.kts` if obfuscation is ever wanted. |
